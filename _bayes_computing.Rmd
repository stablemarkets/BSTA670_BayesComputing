...
$$\begin{align}
	...
	& \approx \frac{1}{T} \sum_{i=1}^T E[\ \tilde y \ | \ \tilde x, \theta_0^{(t)},\theta_1^{(t)},\phi^{(t)}] = \frac{1}{T} \sum_{i=1}^T \theta_0^{(t)} + \theta_1^{(t)}x_i 
\end{align}$$
...
for( i in seq(10, 1000, 10)){
  plot(beta[1,2:i], beta[2,2:i], xlab='theta_1',ylab='theta_2',
...
Motivates the need for approximation methods: find some $q(\theta)$, where $dim(\theta)=p$, such that
...
- **Initialize**: $q(\theta_j)^{(0)}=q_j^{(0)}$ for $j=1, \dots, p$.
- **Update**: for $t=1, \dots, T$.
  - $q_1^{(t)}$ conditional on $q_2^{(t-1)}, \dots, q_p^{(t-1)}$.
  - $q_2$ conditional on $q_1^{(t)}, q_3^{(t-1)}, \dots, q_p^{(t-1)}$
  - $q_3$ conditional on $q_1^{(t)}, q_2^{(t)}, q_4^{(t-1)}, \dots, q_p^{(t-1)}$
  - ...
  - $q_p$ conditional on $q_1^{(t)}, q_2^{(t)}, q_3^{(t)}, \dots, q_p-1^{(t-1)}$
...
- for $t=1, \dots, T$,
  ...
  - $\mu_1^{(t)} = \frac{\sum_ix_{1i}y_i - \mu_0^{(t-1)}\sum_i x_{0i}x_{1i} }{ \sum_i x_{1i}^2 + \frac{\phi}{\tau}}$
